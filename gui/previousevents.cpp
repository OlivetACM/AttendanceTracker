...
void PreviousEvent::getEvents(QDate today) {
    ...
    QString absPath = QStandardPaths::standardLocations(QStandardPaths::DesktopLocation).first() + "/AttendanceTracker/";

    current.setPath(absPath);
    QString todayStr = today.toString("yyyy.MM.dd");
    if (!current.exists()) {
        current.mkdir(current.absolutePath());
    }
    current.setPath(absPath + "attendance/");
    if (!current.exists()) {
        current.mkdir(current.absolutePath());
    }
    current.setPath(current.absolutePath() + "/" + todayStr + "/");
    if (!current.exists()) {
        current.mkdir(current.absolutePath());
    }
